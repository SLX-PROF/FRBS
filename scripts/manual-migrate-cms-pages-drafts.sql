-- Ручное применение src/migrations/20260925_133941_cms_pages_drafts.ts
-- (`npx payload migrate` виснет на инициализации адаптера, см. src/migrations/README.md).
-- Что делает: черновики и версии для товаров, глобалы «Содержимое сайта» (5 страниц) с версиями.
-- ВАЖНО: применять ДО пересборки контейнера с новым кодом. Идемпотентно (можно запускать повторно).
-- Существующие товары остаются опубликованными.
-- На проде:
--   docker compose -f Docker-compose.yml exec -T db psql -U forbsa -d forbsa \
--     < scripts/manual-migrate-cms-pages-drafts.sql
...
CREATE TABLE IF NOT EXISTS "_contacts_page_v" (
  	"id" serial PRIMARY KEY NOT NULL,
...
CREATE TABLE IF NOT EXISTS "_docs_page_v" (
  	"id" serial PRIMARY KEY NOT NULL,
...
CREATE TABLE IF NOT EXISTS "_catalog_page_v" (
  	"id" serial PRIMARY KEY NOT NULL,
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = '_status') THEN
    ALTER TABLE "products" ADD COLUMN "_status" "enum_products_status" DEFAULT 'draft';
    -- Существующие модели остаются опубликованными (иначе колонка по умолчанию сделает их черновиками и они пропадут с сайта).
...
CREATE INDEX IF NOT EXISTS "contacts_page__status_idx" ON "contacts_page" USING btree ("_status");

CREATE INDEX IF NOT EXISTS "_contacts_page_v_version_version__status_idx" ON "_contacts_page_v" USING btree ("version__status");

CREATE INDEX IF NOT EXISTS "_contacts_page_v_created_at_idx" ON "_contacts_page_v" USING btree ("created_at");

CREATE INDEX IF NOT EXISTS "_contacts_page_v_updated_at_idx" ON "_contacts_page_v" USING btree ("updated_at");

CREATE INDEX IF NOT EXISTS "_contacts_page_v_latest_idx" ON "_contacts_page_v" USING btree ("latest");

CREATE INDEX IF NOT EXISTS "docs_page__status_idx" ON "docs_page" USING btree ("_status");

CREATE INDEX IF NOT EXISTS "_docs_page_v_version_version__status_idx" ON "_docs_page_v" USING btree ("version__status");

CREATE INDEX IF NOT EXISTS "_docs_page_v_created_at_idx" ON "_docs_page_v" USING btree ("created_at");

CREATE INDEX IF NOT EXISTS "_docs_page_v_updated_at_idx" ON "_docs_page_v" USING btree ("updated_at");

CREATE INDEX IF NOT EXISTS "_docs_page_v_latest_idx" ON "_docs_page_v" USING btree ("latest");

CREATE INDEX IF NOT EXISTS "catalog_page__status_idx" ON "catalog_page" USING btree ("_status");

CREATE INDEX IF NOT EXISTS "_catalog_page_v_version_version__status_idx" ON "_catalog_page_v" USING btree ("version__status");

CREATE INDEX IF NOT EXISTS "_catalog_page_v_created_at_idx" ON "_catalog_page_v" USING btree ("created_at");

CREATE INDEX IF NOT EXISTS "_catalog_page_v_updated_at_idx" ON "_catalog_page_v" USING btree ("updated_at");

CREATE INDEX IF NOT EXISTS "_catalog_page_v_latest_idx" ON "_catalog_page_v" USING btree ("latest");

CREATE INDEX IF NOT EXISTS "products__status_idx" ON "products" USING btree ("_status");

INSERT INTO "payload_migrations" ("name", "batch")
VALUES ('20260925_133941_cms_pages_drafts', (SELECT COALESCE(MAX(batch), 0) + 1 FROM "payload_migrations"));
